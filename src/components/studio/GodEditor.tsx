...
import {
  getGodCategory,
  godEntitySummary,
  type GodEntity, type GodCategoryDef, type GodCategoryId
} from './godTypes';
import {
...
export const GodEditor: React.FC<Props> = ({ entity, onClose }) => {
  const cat = getGodCategory(entity.category);
  const update = useGodStore(s => s.update);
  const remove = useGodStore(s => s.remove);
  const duplicate = useGodStore(s => s.duplicate);
...
const RelationsField: React.FC<{
  ...
  const target = f.relationTo as GodCategoryId;
  const all = useGodStore(s => s.entities);
  const options = all.filter(e => e.category === target);
  
  let targetName = 'Thực thể';
  try {
    targetName = getGodCategory(target).plural;
  } catch (e) {}
