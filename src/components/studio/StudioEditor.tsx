...
import {
  getCategory, buildDecree,
  asStr, asArr, asStats, asSub, asGraph,
  type StudioEntity, type FieldDef, type FieldValue, type SubItem, type GraphNode,
} from './studioTypes';
...
export const StudioEditor: React.FC<Props> = ({ entity, onClose }) => {
  ...
  const handleDecree = useCallback(() => {
    const current: StudioEntity = { ...entity, name: name.trim(), values };
    update(entity.id, { name: name.trim(), values });
    const text = buildDecree(current, entities);
    useChatStore.getState().setPendingDecree(text);
    useChatStore.getState().setActiveView('chat');
    onClose();
  }, [entity, name, values, entities, update, onClose]);
      ...
      <div className="se-actions">
        {confirmDelete ? (
          ...
          <>
            ...
            >
              <SendIcon size={14} color={cat.accent} /> Ban Xuống Thế Giới
            </button>
          </>
        )}
      </div>
...
/* ── Sublist (cảnh giới / nhánh tiến hóa / công thức) ── */
